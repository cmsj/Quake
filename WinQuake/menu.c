...
#ifdef _WIN32
#define OPTIONS_ITEMS 18 // m3x: was 14
#else
#define OPTIONS_ITEMS 17 // m3x: was 13
#endif
...
void M_AdjustSliders (int dir)
{
  ...
  case 12:  // m3x: toggle shadows
    Cvar_SetValue ("r_shadows", !r_shadows.value);
    break;

  case 13:  // m3x: toggle skybox
    Cvar_SetValue ("r_skybox", !r_skybox.value);
    break;

  case 14:  // m3x: toggle skybox
    Cvar_SetValue ("gl_torchflares", !gl_torchflares.value);
    break;

  case 15:  // m3x: toggle skybox
    Cvar_SetValue ("gl_flashblend", !gl_flashblend.value);
    break;

  case 16:  // m3x: toggle multitexture
    Cvar_SetValue ("gl_multitexture", !gl_multitexture.value);
    if ((gl_mtexable) && (gl_multitexture.value > 0))
...
void M_Options_Draw (void)
{
  ...
  M_DrawCheckbox (220, 128, (int)r_shadows.value);

  // m3x: skybox control
  M_Print (16, 136, "                SkyBox");
  M_DrawCheckbox (220, 136, (int)r_skybox.value);

  // m3x: torch flares control
  M_Print (16, 144, "          Torch Flares");
  M_DrawCheckbox (220, 144, (int)gl_torchflares.value);

  // m3x: flash blend control
  M_Print (16, 152, "           Flash Blend");
  M_DrawCheckbox (220, 152, (int)gl_flashblend.value);

  // m3x: multitexturing control
  M_Print (16, 160, "       Multi Texturing");
  M_DrawCheckbox (220, 160, (int)gl_multitexture.value);
...
void M_Options_Key (int k)
{
  ...
  if (options_cursor == OPTIONS_ITEMS && vid_menudrawfn == NULL)
  {
    if (k == K_UPARROW)
      options_cursor = OPTIONS_ITEMS-1;
    else
