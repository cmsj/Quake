    ...
    if ((gl_mtexable) && (gl_multitexture.value > 0)) {
        for (i=0 ; i<tmax ; i++, dest += stride)
        {
          for (j=0 ; j<smax ; j++)
          {
            ...
            if (t > 255)
              t = 255;
            dest[0] = dest[1] = dest[2] = dest[3] = t; // m3x
            dest += 4;
          }
        }
    }
        ...
        for (i=0 ; i<tmax ; i++, dest += stride)
        {
          for (j=0 ; j<smax ; j++)
          {
            ...
            if (t > 255)
              t = 255;
            //dest[0] = dest[1] = dest[2] = dest[3] = 255-t;
            dest[3] = 255-t;
            dest += 4;
          }
        }
...
void GL_DisableMultitexture(void) 
{
  if (mtexenabled) {
    ...
    GL_SelectTexture(0); //TEXTURE0_SGIS);
    mtexenabled = false;
  }
}
...
{
  if ((gl_mtexable) && (gl_multitexture.value > 0)) { // m3x: modified
    GL_SelectTexture(1); //TEXTURE1_SGIS);
...
void R_DrawSequentialPoly (msurface_t *s)
{
  ...
  if (! (s->flags & (SURF_DRAWSKY|SURF_DRAWTURB|SURF_UNDERWATER) ) )
  {
    ...
    if ((gl_mtexable) && (gl_multitexture.value > 0)) { // m3x: modified
      ...
      GL_Bind (s->lightmaptexturenum);
      glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, (GLint)GL_BLEND);
      ...
      */
      ...
      for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
      {
          qglMTexCoord2fSGIS (GL_TEXTURE0_ARB, v[3], v[4]); // KH
          qglMTexCoord2fSGIS (GL_TEXTURE1_ARB, v[5], v[6]); // KH
          glVertex3fv (v);
      }
      glEnd ();
      return;
  ...
  if ((gl_mtexable) && (gl_multitexture.value > 0)) { // m3x: modified
    ...
    for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
    {
      ...
      qglMTexCoord2fSGIS (GL_TEXTURE1_ARB, v[5], v[6]);
...
R_BlendLightmaps (void)
{
  ...
  {
    glTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, (GLint)GL_MODULATE);
    glColor4f (0,0,0,1);
    glBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);
  }
