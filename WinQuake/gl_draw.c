...
//static const char myver[32] = "$VER: GLQuake AOS4 beta 1\r\n";
...
#else
  glBindTexture(GL_TEXTURE_2D, (GLuint)texnum);
#endif
}
...
#else
  sprintf (ver, "(gl %4.2f beta 7) %4.2f", (float)GLQUAKE_VERSION, (float)VERSION);
#endif
...
void GL_Upload32 (unsigned *data, int width, int height,  qboolean mipmap, qboolean alpha)
{
  ...
  if (! data) { Con_Printf("data error\n"); return; }
  if (! scaled) { Con_Printf("scaled data error\n"); return; }

  //Con_Printf("data %p - size %d %d %d %d\n",data,width,height,mipmap,alpha);
...
void GL_SelectTexture (GLenum target) 
{
  if ((!gl_mtexable) || (gl_multitexture.value == 0)) // m3x: modified
    return;
  qglSelectTextureSGIS(target+GL_TEXTURE0_ARB);
  if (target == oldtarget) 
    return;
